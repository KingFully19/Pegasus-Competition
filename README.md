...
## שלב 3ג: החלפת האייקון (לוגו + favicon)

כדי לשים אייקון משלכם (במקום סמל הכנף שכבר בפרויקט):

1. שימו את קובץ התמונה שלכם בתיקייה `public/`, בשם `emblem.png` בדיוק (תמונה
   ריבועית עם רקע שקוף עובדת הכי טוב).
2. זהו - אותו קובץ משמש גם כלוגו בתוך האתר (תפריט עליון, עמוד הבית, לוח
   המובילים) וגם כאייקון שמופיע בטאב של הדפדפן.
3. אם אתם מעדיפים שם קובץ או סיומת אחרת (למשל `.svg`), עדכנו את הנתיב בשני
   מקומות: בקובץ `index.html` (השורה עם `rel="icon"`) ובקובץ
   `src/components/WingMark.tsx` (השורה עם `src="/pegasus-competition/..."`).
